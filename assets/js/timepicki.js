...
(function ($) {
  $.fn.timepicki = function (options) {
    ...
    return this.each(function () {
      ...
      if (settings.reset) {
        new_ele.append(
          "<div class='buttons'><a href='#' class='reset_time'>Reset</a><a href='#' class='done'>Done</a></div>"
        );
      }
      ...
      inputs.on("keydown", function (e) {
        ...
        if (input.closest(".timepicker_wrap .time").length) {
          ...
          input.closest(".timepicker_wrap .meridian").length &&
      ...
      inputs.on("blur", function () {
        setTimeout(function () {
          ...
          if (
            focused_element.is(":input") &&
            !is_element_in_timepicki(focused_element)
          ) {
            set_value();
            close_timepicki();
          }
      ...
      function is_element_in_timepicki(jquery_element) {
        return (
          $.contains(ele_par[0], jquery_element[0]) ||
          ele_par.is(jquery_element)
        );
      }
      ...
      function set_value(event, close) {
        ...
        if (
          tim.length !== 0 &&
          mini.length !== 0 &&
          (!settings.show_meridian || meri.length !== 0)
        ) {
      ...
      function open_timepicki() {
        ...
        var first_input_exit_handler = function (e) {
          if (e.which === 9 && e.shiftKey) {
            ...
            var previous_form_element = all_form_elements.get(
              index_of_timepicki_input - 1
            );
      ...
      function change_time(cur_ele, direction) {
        var cur_cli = "time";
        var cur_time = Number(
          ele_next.find("." + cur_cli + " .ti_tx input").val()
        );
        var ele_st = Number(settings.min_hour_value);
        var ele_en = Number(settings.max_hour_value);
        var step_size = Number(settings.step_size_hours);
        if (
          (cur_ele && cur_ele.hasClass("action-next")) ||
          direction === "next"
        ) {
          ...
          (cur_ele && cur_ele.hasClass("action-prev")) ||
          direction === "prev"
        ) {
      ...
      function change_mins(cur_ele, direction) {
        var cur_cli = "mins";
        var cur_mins = Number(
          ele_next.find("." + cur_cli + " .mi_tx input").val()
        );
        var ele_st = 0;
        var ele_en = 59;
